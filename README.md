...
HeroFrame is an independent cartoon-production studio for AI generation. Describe a
character once, then generate consistent scenes, multi-shot sequences, versus
matchups, and short animated clips from it — a repeatable production line instead
of one-off prompts.

> Independent project. Not affiliated with, endorsed by, or distributing materials
> from any course or third party. See **Credits** below.

## Features

- **Cast** — create reusable characters (generate a reference, paste a URL, or upload your own art).
- **Scenes** — turn one story beat into a coordinated multi-shot sequence, character-consistent.
- **Variations** — generate multiple takes of a shot to pick the best.
- **Versus Builder** — pick two characters and build an intro + coordinated matchup sequence.
- **Animate** — turn any still into a short video clip (image-to-video).
- **Model picker** — choose image / reference / video models, plus quality presets and style presets.
- **Any-model mode** — call any model id with raw params.
- **Themed background** — a generated, scattered cartoon cast woven into the UI.

## Stack

- Next.js 16 (App Router, TypeScript, Tailwind v4)
- Convex (durable data: characters, generations, workflows)
- Firebase Auth (client auth provider)
- kie.ai (image / video generation) + WaveSpeed (alternate provider)
- Vercel (deployment)

## What ships in v0

- Versus builder workflow template
- Convex schema for projects, workflows, runs, characters, and generations
- API routes for bootstrap, projects, workflows, runs, and generation
- Dashboard UI for:
  - workspace bootstrap
  - run creation
...
3. Add per-model cost and latency analytics for automatic strategy switching.

## Credits & acknowledgements

HeroFrame was inspired by the **Cartoon Hero** course and its community — the
...
audio, or PDFs). All artwork in this repo is generated by the app's own
pipelines. Branded course terms are intentionally avoided. If you are a rights
holder and have any concern, please open an issue and it will be addressed
promptly.
